...
> Frozen 2026-09-04 with 69 open items, when the single `TODOS.md` was replaced by one
> `TODOS/<branch-name>.md` per branch — see `TODOS/README.md`. Nothing new goes here. Sweep it on
> the same schedule as the rest of the directory (promote to an issue, drop, or leave), and delete
> the file when it is empty. Code comments that say "recorded in TODOS.md" mean this file.
