...
member's Base Camp `user.id`/`username`) are already in the summary payload, and the call uses the
same origin/session/auth. It returns two parts:

- **`blocks`** — the full course tree (`course → chapters (Level 1..5, Path Completion) →
  sequentials (projects)`). Each **project carries `display_name` and a per-project
  `complete: true|false`**, plus `block_lib_type: "imported" | "elective"`. Unchosen electives
  appear as placeholders (empty `block_id`, generic name like `"2nd Elective"`). Chapters carry
  `complete` and `min_req_electives`.
...
2. **`/detail`'s `blocks` tree becomes the catalog's source of truth.** `pathways_projects` is
   *derived* from Base Camp, keyed by `block_id`, retiring the hand-seeded PDF catalog. This makes
   per-project completion join by exact `block_id` — no fragile name-matching — and yields elective
   structure (`min_req_electives`, elective vs required) for free.
...
### Catalog — now derived from `/detail`
...
pathways_projects {
  ...
  level int (1–5),
  name,
  bcm_block_id text UNIQUE,   -- exact join key from /detail blocks
  is_elective boolean,        -- from block_lib_type: "elective" vs "imported"
  sort_order
}
...
pathways_path_levels {        -- NEW
  ...
  path_id → pathways_paths,
  level int,
  min_req_electives int,
  UNIQUE(path_id, level)
}
```

- `pathways_projects` is **upserted from the `blocks` tree**, keyed by `bcm_block_id`. A project
  rename in Base Camp updates the same row (block_id is stable).
- **Elective placeholders** (empty `block_id`, e.g. `"2nd Elective"`) are **not** catalog rows —
  they represent an unchosen slot. The requirement count lives in
  `pathways_path_levels.min_req_electives` instead.
- `is_elective` (from `block_lib_type`) replaces the old `is_required` display-emphasis flag.
- Hand-seeding from the community PDF is **retired**.
...
- Fully re-derived every sync; read-only from the app's perspective.
...
BcmDetailPayload = { courseId, basecampUserId, blocks, speeches }
```
...
- **`details` is optional** for backward compatibility: an older extension build (e.g. the one
  ...
  upgrade.
- **Two new pure functions** in `-logic.ts` siblings (bundle-leak guard — never imported by client
  code):
  - one derives catalog upserts (`pathways_projects` + `pathways_path_levels`) from the `blocks`
    trees;
  - one produces `bcm_project_progress` rows by joining `blocks[].complete` with the `speeches`
    map, resolved to enrollments via `basecampUserId` + `courseId`.
- **Ordering within a sync:** summary parse first (creates/updates `path_enrollments` +
  `basecamp_user_id`), then catalog upsert, then per-member detail rows that FK into those
  enrollments.
- **Unmatched detail** (member/path with no enrollment) is **skipped and surfaced** in the result,
  same discipline as unmatched summary rows. `SyncResult` gains detail counters (projects upserted,
  members with detail, unmatched detail).

## Extension detail fan-out

The content script (from #107/#119) gains a second phase after the summary walk:

- From the summary pages it already holds, enumerate `(basecampUserId, courseId)` pairs — the exact
  member×path set.
- For each, `GET …/api/bcm/progress/<courseId>/detail?user=<guid>&page_size=5000` (one page each),
  same origin/session/headers as the summary calls — no new auth path, CORS/token story unchanged.
- **Throttle** the fan-out with small bounded concurrency (2–3 in flight). **Graceful per-call
  failure:** a member whose detail call fails is omitted from `details` (their counts still sync
  from the summary); one bad call never aborts the sync.
- POST summary `pages` + collected `details` in the existing single request → "one button,
  always-fresh" UX preserved.

This is a **new extension version, sequenced after #119** (landed) — explicitly **not folded into
#119**.

## Identity, PII & security

- **Identity join:** detail rows resolve via `basecamp_user_id` + `courseId` → the
  `path_enrollments` row the summary pass just created/updated. Reuses ADR-0008's email→GUID match;
  no new identity mechanism.
- **PII (established rules, carried forward):** raw `.har`/detail captures stay **gitignored** —
  they contain member PII *and* session tokens; **no Base Camp session token is ever persisted**;
  test fixtures are **scrubbed**. Storing speech titles in `bcm_project_progress` is consistent with
  already storing member-entered titles in `speeches` — no new privacy category.

## Testing

- Detail parser, catalog derivation, and completion upsert: **integration-tested against a scrubbed
  `/detail` fixture** in the `tm_test` DB, with logic in `*-logic.ts` siblings (the
  `server-modules.guard.test.ts` bundle-leak guard applies).
- Coverage: catalog upsert by `block_id` (incl. rename → same row); elective placeholders excluded;
  `min_req_electives` captured; complete-without-speech; speech title/date join; unmatched-detail
  reporting; and **backward compatibility** — a summary-only payload (no `details`) behaves exactly
  as today.

## Phasing

- **Slice 1 (this spec's plan) — backend + extension.** Mirror table + catalog derivation schema,
  the extended ingest contract, the detail parser/upsert, and the extension fan-out. Produces synced
  authoritative data end-to-end, testable, with **no UI change** yet.
- **Slice 2 (fast-follow) — UI switchover.** "Your wins" reads `bcm_project_progress`
  ...
  `min_req_electives` for elective slots; a real delivered-speech history from the mirror. All read
  the mirror **when present, falling back to today's inference** when an enrollment has no detail
  rows yet.
...
- **ADR-0011** — "Base Camp `/detail`: authoritative per-project completion + speeches" — records
  the reversal of the two assumptions and the derive-catalog-from-Base-Camp decision.
