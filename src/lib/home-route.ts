...
export type HomePath = "/officers" | "/dashboard";
 ...
 * their own dashboard (#542 — the roster is a manage surface, not a member
 * home). Mirrors the `isOfficer` rule in the authed shell (`_authed.tsx`).
 */
export function homeRedirectTarget(input: {
	...
	return isOfficer ? "/officers" : "/dashboard";
}
