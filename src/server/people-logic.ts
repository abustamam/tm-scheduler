...
export async function mergePeople(
	...
	return db.transaction(async (tx) => {
		...
		// 4. Delete the absorbed Person. Done BEFORE the keeper reconcile below so
		//    adopting the absorbed's `customer_id` / `basecamp_user_id` (both
		//    non-deferrable UNIQUE) can't collide with the still-live absorbed row.
		...
		//    — the superadmin holds no membership in the club).
		const movedCounts = {
